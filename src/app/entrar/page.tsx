...
export default function EntrarPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (error || !data.session?.access_token) {
      setErrorMessage(error?.message ?? "Não foi possível iniciar sessão.");
    ...
    if (!response.ok) {
      setErrorMessage(await readApiErrorMessage(response, "Não foi possível carregar seus dados."));
      setIsSubmitting(false);
      return;
    }

    const me = (await response.json()) as MeResponse;
    router.replace(resolvePostLoginRoute(me.nextStep, redirectTo));
  }
      ...
      <main className="flex min-h-[calc(100vh-5rem)] items-start justify-center px-5 pb-16 pt-8 sm:items-center sm:px-6 sm:pt-4 lg:px-8">
        <div className="w-full max-w-md">
          ...
          <p className="mt-7 text-center text-[0.88rem] text-white/55">
            Ainda não tem conta?{" "}
            <Link href={signupHref} className="font-semibold text-[#f26a1b] transition hover:text-[#ff7d35]">
              Criar conta
            </Link>
          </p>
