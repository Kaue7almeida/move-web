...
async function readPostTrainerWorkoutBody(request: Request) {
  try {
    const body: unknown = await request.json();

    return parsePostTrainerWorkoutBody(body);
  } catch (error: unknown) {
...
export async function GET(request: Request) {
  try {
    ...
    const result = await workoutService.listTrainerWorkouts({
...
export async function POST(request: Request) {
  try {
    const authContext = await ensureAuthenticated(request);
    const body = await readPostTrainerWorkoutBody(request);
    const workoutService = makeWorkoutService();
    const result = await workoutService.createTrainerWorkout(
      {
        userId: authContext.userId,
        email: authContext.email,
      },
