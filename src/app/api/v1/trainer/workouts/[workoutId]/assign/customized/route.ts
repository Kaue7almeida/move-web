...
import { parsePostCustomizedAssignWorkoutBody } from "./schema";
...
async function readPostCustomizedAssignWorkoutBody(request: Request) {
  try {
    const body: unknown = await request.json();

    return parsePostCustomizedAssignWorkoutBody(body);
  } catch (error: unknown) {
...
export async function POST(
  request: Request,
  context: RouteContext<"/api/v1/trainer/workouts/[workoutId]/assign/customized">,
) {
  try {
    const authContext = await ensureAuthenticated(request);
    const workoutId = await readWorkoutId(context.params);
    const body = await readPostCustomizedAssignWorkoutBody(request);
    const workoutService = makeWorkoutService();
    const result = await workoutService.assignCustomizedWorkoutToStudent(
      {
